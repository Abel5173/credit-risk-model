...
class WOEIVTransformer(BaseEstimator, TransformerMixin):
    """
    Uses xverse's WOE transformer for WOE encoding. IV-based feature selection is not used.
    """
...
def build_feature_engineering_pipeline(
    df: pd.DataFrame, scaler: str = 'standard', use_woe_iv: bool = True, min_iv: float = 0.02
):
    """
    ...
    if use_woe_iv:
        steps.append(
            ('woe_iv', WOEIVTransformer(target_col='FraudResult', min_iv=min_iv))
        )
